...
document.addEventListener('partials:loaded', async () => {
  ...
  async function reviewRequest(requestId, requests, onComplete){
    ...
    try {
      ...
      } else if (['payment_received', 'confirmed', 'under_admin_review'].includes(request.status)) {
  ...
  function paymentReviewCard(request){
    ...
    const canRelease = ['payment_received', 'confirmed', 'under_admin_review'].includes(request.status) && request.proof?.status === 'approved';
    const release = canRelease ? `<button class="btn btn-primary btn-sm payment-review-action" data-id="${request.id}" data-action="release-funds">Release Funds</button>` : '';
    const refund = ['disputed', 'payment_pending', 'payment_proof_submitted'].includes(request.status) ? `<button class="btn btn-danger btn-sm payment-review-action" data-id="${request.id}" data-action="refund-funds">Refund Requester</button>` : '';
    const proof = request.proof ? `<div class="proof-preview"><img src="${request.proof.url}" alt="Payment proof for ${request.id}" loading="lazy"><a href="${request.proof.url}" target="_blank" rel="noreferrer">Open full image</a></div>` : 'Not submitted';
    return `<div class="card request-card admin-review-card"><div class="rc-top"><div><div class="cell-primary">${request.id}</div><div class="cell-muted">${request.requester} → ${request.fulfiller || 'Unassigned receiver'}</div></div>${SC.statusBadge(request.status)}</div><div class="kv-list"><div class="kv-row"><span class="k">Method</span><span class="v">${SC.methodMeta(request.method).label}</span></div><div class="kv-row"><span class="k">Amount</span><span class="v">${SC.fmtMoney(request.amount)}</span></div><div class="kv-row"><span class="k">Reward + fee</span><span class="v">${SC.fmtMoney(request.reward)} + ${SC.fmtMoney(request.fee)}</span></div><div class="kv-row"><span class="k">Escrow</span><span class="v">${request.escrowAsset} · ${request.depositStatus}</span></div><div class="kv-row"><span class="k">Proof</span><span class="v">${proof}</span></div><div class="kv-row"><span class="k">Reference</span><span class="v">${request.proof?.transactionReference || '—'}</span></div><div class="kv-row"><span class="k">Note</span><span class="v">${request.proof?.note || '—'}</span></div></div><div class="table-actions admin-review-actions" style="margin-top:18px; flex-wrap:wrap;">${action}${release}${refund}</div></div>`;
  }
  ...
  async function handlePaymentReviewAction(button, requests){
    ...
    if (action === 'refund-funds' && !window.confirm(`Refund the escrow funds to ${request.requester}?\n\nAmount: ${SC.fmtMoney(request.amount + request.reward)}\nAsset: ${request.escrowAsset}\nRequest ID: ${request.id}`)) return;
