...
const SCStore = (() => {
  ...
  async function update(id, changes){
    let payload;
    if (changes.sandboxFund) payload = await api(`/api/requests/${id}/fund`, { method: 'POST', body: JSON.stringify({ sandbox: true }) });
    else if (changes.txHash) payload = await api(`/api/requests/${id}/fund`, { method: 'POST', body: JSON.stringify({ txHash: changes.txHash }) });
