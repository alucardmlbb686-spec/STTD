...
- Render mounts the persistent `starcurrency-uploads` disk at `/opt/render/project/src/uploads`, preserving payment proof and chat files across deploys and restarts. Existing proof records created before this disk was attached may need to be uploaded again if their old local files were already removed.
- `UPLOADS_DIR` points the upload handlers at that mounted disk; keep it aligned with the Render disk `mountPath` when deploying elsewhere.
